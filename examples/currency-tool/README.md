...
A GoMind tool that provides currency conversion using the [Frankfurter](https://frankfurter.dev/) API. This tool is independent and can be deployed standalone - it only requires Redis for service discovery.
...
Running this tool locally demonstrates how GoMind tools register with Redis for service discovery and expose capabilities via a REST API.

### Prerequisites

Before running this example, you need to install the following tools. Choose the instructions for your operating system.

> **Note:** No API key is required. This tool uses the free [Frankfurter](https://frankfurter.dev/) API.
...
# 2. Deploy the tool (includes cluster and infrastructure if needed)
./setup.sh deploy
```
...
|---------|-----|-------------|
| **Currency Tool API** | http://localhost:8097 | REST API for currency conversion |
...
│                            (Port 8097)                                   │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────────┐   │
│  │  Service         │  │   Capability     │  │   Frankfurter API    │   │
│  │  Registration    │  │   Handlers       │  │   Client             │   │
│  │  (Redis)         │  │                  │  │                      │   │
│  └──────────────────┘  └──────────────────┘  └──────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
                                 │
...
4. **Tool calls** the Frankfurter API for currency data
...
The defaults work for local Kubernetes development with no modifications needed.
...
kubectl exec -n gomind-examples deploy/redis -- redis-cli ping
```

**2. Tool not discovered by agents**
...
```bash
pkill -f 'kubectl.*port-forward.*currency-tool'
./setup.sh forward
```
...
# Check pod status
kubectl get pods -n gomind-examples -l app=currency-tool
