...
func main() {
	...
	framework, err := core.NewFramework(tool,
		...
		core.WithMiddleware(telemetry.TracingMiddleware("currency-tool")),
	)
	if err != nil {
		log.Fatalf("Failed to create framework: %v", err)
	}

	log.Println("Currency Tool Service Starting...")
	log.Println("Telemetry: Enabled")
	log.Printf("Server Port: %d\n", port)
	log.Println("API: Frankfurter (free, no API key required)")
	log.Println()
